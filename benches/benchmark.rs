...
fn bench_names(c: &mut criterion::Criterion) {
    ...
    c.bench_function("names_ref", |b| b.iter(names_ref));
...
pub fn names_ref() {
    let m =
        MetadataRef::from("[ www.UsaBit.com ] - My Cousin Vinny (1992) BluRay 720p 750MB Ganool")
            .unwrap();
    assert_eq!(m.year(), Some(1992));
    assert_eq!(m.title(), "My Cousin Vinny");

    let m = MetadataRef::from("2012.2009.1080p.BluRay.x264.DTS-METiS").unwrap();
    assert_eq!(m.year(), Some(2009));
    assert_eq!(m.title(), "2012");
    let m = MetadataRef::from(
        "[TorrentCounter.to].Pacific.Rim.2.Uprising.2018.1080p.HC.HDRip.x264.[2GB]",
    )
    .unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    ...
    let m = MetadataRef::from("22 Jump Street (2014) 720p BrRip x264 - YIFY").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "22 Jump Street");

    let m = MetadataRef::from("Hercules.2014.EXTENDED.1080p.WEB-DL.DD5.1.H264-RARBG").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "Hercules");
    ...
    let m = MetadataRef::from("Hercules (2014) WEBDL DVDRip XviD-MAX").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "Hercules");

    let m = MetadataRef::from("WWE Hell in a Cell 2014 PPV WEB-DL x264-WD -={SPARROW}=-").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "WWE Hell in a Cell");

    let m = MetadataRef::from("UFC.179.PPV.HDTV.x264-Ebi[rartv]").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), None);
    assert_eq!(m.title(), "UFC 179");
    ...
    let m =
        MetadataRef::from("Marvels Agents of S H I E L D S02E05 HDTV x264-KILLERS [eztv]").unwrap();
    assert_eq!(m.season(), Some(2));
    assert_eq!(m.episode(), Some(5));
    assert_eq!(m.year(), None);
    assert_eq!(m.title(), "Marvels Agents of S H I E L D");
    ...
    let m =
        MetadataRef::from("X-Men.Days.of.Future.Past.2014.1080p.WEB-DL.DD5.1.H264-RARBG").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "X-Men Days of Future Past");

    let m = MetadataRef::from("Guardians Of The Galaxy 2014 R6 720p HDCAM x264-JYK").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "Guardians Of The Galaxy");

    let m = MetadataRef::from("Marvel\'s.Agents.of.S.H.I.E.L.D.S02E01.Shadows.1080p.WEB-DL.DD5.1")
        .unwrap();
    assert_eq!(m.season(), Some(2));
    assert_eq!(m.episode(), Some(1));
    assert_eq!(m.year(), None);
    ...
    let m =
        MetadataRef::from("Marvels Agents of S.H.I.E.L.D. S02E06 HDTV x264-KILLERS[ettv]").unwrap();
    assert_eq!(m.season(), Some(2));
    assert_eq!(m.episode(), Some(6));
    assert_eq!(m.year(), None);
    assert_eq!(m.title(), "Marvels Agents of S.H.I.E.L.D.");

    let m = MetadataRef::from("Guardians of the Galaxy (CamRip / 2014)").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "Guardians of the Galaxy");

    let m = MetadataRef::from("The.Walking.Dead.S05E03.1080p.WEB-DL.DD5.1.H.264-Cyphanix[rartv]")
        .unwrap();
    assert_eq!(m.season(), Some(5));
    assert_eq!(m.episode(), Some(3));
    assert_eq!(m.year(), None);
    ...
    let m = MetadataRef::from("Brave.2012.R5.DVDRip.XViD.LiNE-UNiQUE").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2012));
    assert_eq!(m.title(), "Brave");

    let m = MetadataRef::from("Lets.Be.Cops.2014.BRRip.XViD-juggs[ETRG]").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "Lets Be Cops");

    let m = MetadataRef::from("These.Final.Hours.2013.WBBRip XViD").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2013));
    assert_eq!(m.title(), "These Final Hours");

    let m = MetadataRef::from("Downton Abbey 5x06 HDTV x264-FoV [eztv]").unwrap();
    assert_eq!(m.season(), Some(5));
    assert_eq!(m.episode(), Some(6));
    assert_eq!(m.year(), None);
    assert_eq!(m.title(), "Downton Abbey");

    let m = MetadataRef::from("Annabelle.2014.HC.HDRip.XViD.AC3-juggs[ETRG]").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "Annabelle");
    ...
    let m = MetadataRef::from("Lucy.2014.HC.HDRip.XViD-juggs[ETRG]").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "Lucy");
    ...
    let m = MetadataRef::from("The Flash 2014 S01E04 HDTV x264-FUM[ettv]").unwrap();
    assert_eq!(m.season(), Some(1));
    assert_eq!(m.episode(), Some(4));
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "The Flash");

    let m = MetadataRef::from("South Park S18E05 HDTV x264-KILLERS [eztv]").unwrap();
    assert_eq!(m.season(), Some(18));
    assert_eq!(m.episode(), Some(5));
    assert_eq!(m.year(), None);
    assert_eq!(m.title(), "South Park");

    let m = MetadataRef::from("The Flash 2014 S01E03 HDTV x264-LOL[ettv]").unwrap();
    assert_eq!(m.season(), Some(1));
    assert_eq!(m.episode(), Some(3));
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "The Flash");

    let m = MetadataRef::from("The Flash 2014 S01E01 HDTV x264-LOL[ettv]").unwrap();
    assert_eq!(m.season(), Some(1));
    assert_eq!(m.episode(), Some(1));
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "The Flash");

    let m = MetadataRef::from("Lucy 2014 Dual-Audio WEBRip 1400Mb").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "Lucy");

    let m = MetadataRef::from("Teenage Mutant Ninja Turtles (HdRip / 2014)").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "Teenage Mutant Ninja Turtles");
    ...
    let m =
        MetadataRef::from("Teenage Mutant Ninja Turtles (unknown_release_type / 2014)").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.year(), Some(2014));
    assert_eq!(m.title(), "Teenage Mutant Ninja Turtles");

    let m = MetadataRef::from("The Simpsons S26E05 HDTV x264 PROPER-LOL [eztv]").unwrap();
    assert_eq!(m.season(), Some(26));
    assert_eq!(m.episode(), Some(5));
    assert_eq!(m.year(), None);
    assert_eq!(m.title(), "The Simpsons");
    ...
    let m =
        MetadataRef::from("The EXPANSE - S03 E01 - Fight or Flight (1080p - AMZN Web-DL)").unwrap();
    assert_eq!(m.season(), Some(3));
    assert_eq!(m.episode(), Some(1));
    assert_eq!(m.title(), "The EXPANSE");
    assert_eq!(m.resolution(), Some(Resolution::R1080p));
    ...
    let m = MetadataRef::from("Fargo.S04E03.WEB.x264-PHOENiX[TGx]").unwrap();
    assert_eq!(m.season(), Some(4));
    assert_eq!(m.episode(), Some(3));
    assert_eq!(m.title(), "Fargo");
    assert_eq!(m.quality(), Some(Quality::Web));
    ...
    let m = MetadataRef::from("Yes Day (2021) [h265 WEBDL-1080p] [tt8521876]").unwrap();
    assert_eq!(m.season(), None);
    assert_eq!(m.episode(), None);
    assert_eq!(m.imdb_tag(), Some("tt8521876"));
    assert_eq!(m.year(), Some(2021));
